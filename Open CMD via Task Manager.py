...
# Add delay, waiting for the Task Manager to open
if (ControlCodes.upper() == "HONEYWELL") | (ControlCodes.upper() == "EYOYO"):
    TxString = TxString + Ins * (18 * 8)
else: TxString = TxString + Ins * (24 * 8)

# Add control operations to pass the control to the Task Manager
TxString = (TxString + AltBackTab) # Press Alt-Back Tab to pass the control to the Task Manager window

# Add delay, waiting for the control to pass to the Task Manager
if (ControlCodes.upper() == "HONEYWELL") | (ControlCodes.upper() == "EYOYO"):
    TxString = TxString + Ins * (2 * 8)
else: TxString = TxString + Ins * (8 * 8)

# This script assumes that the task manager is displaying detailed information.
# If this is not the case, then an ALT-D command is required to command the task
# manager to display more details before the ALT-F command.

# Add command to open the file menu and run a new task
TxString = (TxString +
            Alt+'f'+AltRelease+ # Open the File menu
            'n') # Select Run new task

# Add delay, waiting for the new task box to open
if (ControlCodes.upper() == "HONEYWELL") | (ControlCodes.upper() == "EYOYO"):
    TxString = TxString + Ins * (8 * 8)
else: TxString = TxString + Ins * (32 * 8)

# Add the command-line command
TxString = (TxString +
            'cmd'+Enter) # Open the command-line window
...
draw.text((0, 0), ControlCodes + ':' + 'Open CMD via Task Manager',fill = 'black', font = font)
